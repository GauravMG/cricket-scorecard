...
function renderScore(api) {
    ...
    if (inningsList.length) {
        ...
        if (t1) {
            scoreA = `${t1.score}/${t1.wickets}`;
            oversA = `${t1.overs} OV`;
        }

        if (t2) {
            scoreB = `${t2.score}/${t2.wickets}`;
            oversB = `${t2.overs} OV`;
        }
    }
    ...
    updateScoreboard({
        ...
        // scoreA: Object.keys(matchDetails).length ? scoreA : "",
        // oversA: Object.keys(matchDetails).length ? oversA : "",
        // scoreB: Object.keys(matchDetails).length ? battingTeamShort === header.team1?.shortName ? "Batting" : "Bowling" : "",
        scoreA,
        oversA,
        scoreB,
        oversB,
        ...
        matchSub: matchDetails.state === "Complete"
            ? header.status
            : mini.status || "-",
        ...
        reqRate: matchDetails.state === "Complete"
            ? "-"
            : (mini.requiredRunRate || "-"),

        // -------- BATTERS --------
        bat1Name: striker.name || "-",
        bat1Stat: Object.keys(matchDetails).length ? `${striker.runs || 0} (${striker.balls || 0}) • ${striker.strikeRate || 0}` : "",

        bat2Name: nonStriker.name || "-",
        bat2Stat: Object.keys(matchDetails).length ? `${nonStriker.runs || 0} (${nonStriker.balls || 0}) • ${nonStriker.strikeRate || 0}` : "",

        batNote: partnership,

        // -------- BOWLERS --------
        bowl1Name: bowler1.name || "-",
        bowl1Stat: Object.keys(matchDetails).length ? `${bowler1.wickets || 0}-${bowler1.runs || 0} (${bowler1.overs || 0}) • ${bowler1.economy || 0}` : "",

        bowl2Name: bowler2.name || "-",
        bowl2Stat: Object.keys(matchDetails).length ? `${bowler2.wickets || 0}-${bowler2.runs || 0} (${bowler2.overs || 0}) • ${bowler2.economy || 0}` : "",

        bowlNote: Object.keys(matchDetails).length ? matchDetails.state === "Complete" ? "Match Complete" : "Live Bowling" : "",

        // -------- MATCH INFO --------
        // inn1: Object.keys(matchDetails).length ? matchDetails.inningsScoreList?.length === 2 ? "2nd" : "1st" : "",
        ...
        ticker: header.status || mini.status || "-",
    });
}
